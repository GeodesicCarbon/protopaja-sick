...
struct area_t {
								...
								bool test_poi(poi_t &poi) {
																if (poi.poi_pos.first > this->x_min && poi.poi_pos.first < this->x_max
																				&& poi.poi_pos.second > this->y_min && poi.poi_pos.second < this->y_min) {
																								this->is_active = true;
																								return true;
...
void visualize_areas() {
								...
								for(auto &area : area_repository) {
																ROS_INFO("Drawing %s", area.name.c_str());
...
float point_distance(std::pair<float,float> from, std::pair <float,float> to){
								return std::sqrt(std::pow(from.first + to.first,2) + std::pow(from.first + to.first,2));
}
...
{
								if(main_poi) {
																ROS_INFO("Main POI instantiated");
								}
								...
								if (client_pointer->call(srv))
								{
																...
																if (srv.response.mobiles_x.size() > 0) {
																								...
																								if(smallest_index < srv.response.mobiles_x.size() && smallest_index < srv.response.mobiles_y.size()) {
																																...
																																ROS_INFO("Found main_poi: [%.2f %.2f]", main_poi->poi_pos.first,
																																									main_poi->poi_pos.second);
																								} else {
																																ROS_INFO("Something wrong with service call");
																								}
																}
